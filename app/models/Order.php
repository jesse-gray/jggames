  ...
  class Order {
    ...
    public function addNewOrder($data){
        ...
        $this->db->query('  SELECT 
                            ...
                            WHERE cart_product.user_id = :user_id;');
        // Bind values
        ...
        foreach($results as $result){
            $this->db->query('  INSERT INTO order_line
    ...
    public function getOrdersByUserId($user_id){      
      $this->db->query('SELECT `"order"`.id AS order_id, 
                        `"order"`.total, 
                        `"order"`.placed_date, 
                        `"order"`.shipped_date, 
                        address.* 
    ...
    public function getOrderById($id){
      $this->db->query('SELECT `"order"`.id AS order_id, 
                        ...
                        LEFT JOIN products 
                        ON order_line.product_id = products.id 
                        LEFT JOIN users 
    ...
    public function getOrders(){
      $this->db->query('SELECT `"order"`.id AS order_id, users.name AS username, `"order"`.total AS order_total, `"order"`.placed_date AS order_placed_date FROM `"order"` JOIN users ON `"order"`.user_id = users.id');
